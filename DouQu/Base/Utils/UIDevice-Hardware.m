...
// 如果拿不到有效的地址或者是iOS7，返回idfa
- (NSString *)macaddress
{
    NSString *macaddress = nil;
    if ([UIDevice currentDevice].systemVersion.floatValue < 7)
    {
        macaddress = [self realMacAddress];
    }
    if (!macaddress)
    {
        macaddress = [self idfaString];
        if (!macaddress)
        {
            macaddress = [self openUDID];
        }
    }
    if (macaddress == nil || macaddress.length == 0)
    {
        macaddress = @"cannot get macaddress and IDFA and OpenUDID";
    }
    return macaddress;
}
...
- (NSString *)realMacAddress
{
    ...
    if (outstring && ![outstring hasPrefix:@"02:00:00"])
    {
        realMacAddress = outstring;
    }
    if (buf)
    {
        free(buf);
...
- (NSString *)openUDID
{
    ...
    if ([OpenUDID value].length > 0)
    {
        openUDID = [NSString stringWithFormat:@"oid-%@", [OpenUDID value]];
    }
    return openUDID;
}
...
- (NSString *)idfaString
{
    ...
    if (adMgr.isAdvertisingTrackingEnabled && adMgr.advertisingIdentifier.UUIDString)
    {
